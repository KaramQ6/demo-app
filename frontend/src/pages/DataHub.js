                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                    {citiesData.map((city, index) => (
                        <div key={index} className="animate-fade-in-up" style={{ animationDelay: `${index * 0.1}s` }}>
                            <Card className="glass-card h-full overflow-hidden border-white/20 bg-white/10 backdrop-blur-md shadow-xl">
                                <CardHeader>
                                    <CardTitle className="flex items-center justify-between text-2xl font-semibold text-white drop-shadow">
                                ...
                                <CardContent className="flex flex-col items-center justify-center space-y-4 pt-0">
                                    <div className="text-6xl font-bold text-purple-400 drop-shadow">
                                        {Math.round(city?.main?.temp || city?.temperature || 0)}°C
                                    </div>
                                    <div className="flex items-center space-x-4 rtl:space-x-reverse text-gray-200">
                                    ...
                                    {(city?.main?.humidity || city?.humidity) && (
                                        <div className="flex items-center space-x-2 rtl:space-x-reverse text-gray-300 pt-2">
