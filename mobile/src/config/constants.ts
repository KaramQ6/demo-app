// API Configuration
export const API_CONFIG = {
  BASE_URL: 'http://46.101.127.177:8001',
  API_PREFIX: '/api',
  TIMEOUT: 10000,
  RETRY_ATTEMPTS: 3,
};

// Supabase Configuration
export const SUPABASE_CONFIG = {
  URL: 'https://shhmlmejukbvtotgjule.supabase.co',
  ANON_KEY: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InNoaG1sbWVqdWtidnRvdGdqdWxlIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTQxNzc0NTUsImV4cCI6MjA2OTc1MzQ1NX0.qvcmpysmFjPwtQRl0ueuCtcx6SckBbS73wzDqiBsN8E',
};

// Storage Keys
export const STORAGE_KEYS = {
  USER_TOKEN: 'userToken',
  USER_DATA: 'userData',
  USER_PREFERENCES: 'userPreferences',
  LANGUAGE: 'language',
  OFFLINE_ACTIONS: 'offlineActions',
  CACHED_DATA: 'cachedData',
  LAST_SYNC: 'lastSync',
};

// App Configuration
export const APP_CONFIG = {
  NAME: 'SmartTour.Jo',
  VERSION: '1.0.0',
  BUNDLE_ID: 'com.smarttour.jo',
  SUPPORTED_LANGUAGES: ['en', 'ar'],
  DEFAULT_LANGUAGE: 'en',
  RTL_LANGUAGES: ['ar'],
};

// Map Configuration
export const MAP_CONFIG = {
  JORDAN_COORDINATES: {
    latitude: 31.9539,
    longitude: 35.9106,
    latitudeDelta: 2.0,
    longitudeDelta: 2.0,
  },
  DEFAULT_ZOOM: 10,
  MARKER_CLUSTER_RADIUS: 50,
};

// Feature Flags
export const FEATURES = {
  OFFLINE_MODE: true,
  PUSH_NOTIFICATIONS: true,
  BIOMETRIC_AUTH: true,
  DARK_MODE: true,
  AR_VIEW: false, // Will be enabled later
  VOICE_ASSISTANT: false, // Will be enabled later
};

// Cache Configuration
export const CACHE_CONFIG = {
  MAX_AGE: 24 * 60 * 60 * 1000, // 24 hours in milliseconds
  MAX_SIZE: 50 * 1024 * 1024, // 50MB in bytes
  IMAGE_CACHE_SIZE: 100, // Number of images to cache
};

// Validation Rules
export const VALIDATION = {
  EMAIL_REGEX: /^[^\s@]+@[^\s@]+\.[^\s@]+$/,
  PASSWORD_MIN_LENGTH: 6,
  USERNAME_MIN_LENGTH: 3,
  MAX_UPLOAD_SIZE: 10 * 1024 * 1024, // 10MB
};

// Jordan Tourist Destinations
export const JORDAN_DESTINATIONS = [
  {
    id: 'petra',
    name: 'Petra',
    name_ar: 'البتراء',
    description: 'Ancient city carved into red sandstone cliffs',
    description_ar: 'مدينة أثرية محفورة في الصخر الرملي الأحمر',
    latitude: 30.3285,
    longitude: 35.4444,
    category: 'historical',
    image_url: 'https://example.com/petra.jpg',
    rating: 4.9,
    is_active: true,
    created_at: new Date().toISOString(),
  },
  {
    id: 'wadi-rum',
    name: 'Wadi Rum',
    name_ar: 'وادي رم',
    description: 'Protected desert wilderness',
    description_ar: 'محمية صحراوية برية',
    latitude: 29.5324,
    longitude: 35.4206,
    category: 'nature',
    image_url: 'https://example.com/wadi-rum.jpg',
    rating: 4.8,
    is_active: true,
    created_at: new Date().toISOString(),
  },
  {
    id: 'dead-sea',
    name: 'Dead Sea',
    name_ar: 'البحر الميت',
    description: 'Lowest point on Earth',
    description_ar: 'أخفض نقطة على الأرض',
    latitude: 31.5590,
    longitude: 35.4732,
    category: 'nature',
    image_url: 'https://example.com/dead-sea.jpg',
    rating: 4.7,
    is_active: true,
    created_at: new Date().toISOString(),
  },
  {
    id: 'jerash',
    name: 'Jerash',
    name_ar: 'جرش',
    description: 'Ancient Roman city',
    description_ar: 'مدينة رومانية قديمة',
    latitude: 32.2744,
    longitude: 35.8906,
    category: 'historical',
    image_url: 'https://example.com/jerash.jpg',
    rating: 4.6,
    is_active: true,
    created_at: new Date().toISOString(),
  },
  {
    id: 'amman-citadel',
    name: 'Amman Citadel',
    name_ar: 'جبل القلعة',
    description: 'Historic citadel in Amman',
    description_ar: 'قلعة تاريخية في عمان',
    latitude: 31.9539,
    longitude: 35.9345,
    category: 'historical',
    image_url: 'https://example.com/amman-citadel.jpg',
    rating: 4.5,
    is_active: true,
    created_at: new Date().toISOString(),
  },
  {
    id: 'aqaba',
    name: 'Aqaba',
    name_ar: 'العقبة',
    description: 'Red Sea coastal city',
    description_ar: 'مدينة ساحلية على البحر الأحمر',
    latitude: 29.5267,
    longitude: 35.0067,
    category: 'coastal',
    image_url: 'https://example.com/aqaba.jpg',
    rating: 4.4,
    is_active: true,
    created_at: new Date().toISOString(),
  },
  {
    id: 'dana-reserve',
    name: 'Dana Biosphere Reserve',
    name_ar: 'محمية ضانا الطبيعية',
    description: 'Largest nature reserve in Jordan',
    description_ar: 'أكبر محمية طبيعية في الأردن',
    latitude: 30.6750,
    longitude: 35.5850,
    category: 'nature',
    image_url: 'https://example.com/dana.jpg',
    rating: 4.3,
    is_active: true,
    created_at: new Date().toISOString(),
  },
  {
    id: 'ajloun-castle',
    name: 'Ajloun Castle',
    name_ar: 'قلعة عجلون',
    description: 'Medieval castle',
    description_ar: 'قلعة من العصور الوسطى',
    latitude: 32.3328,
    longitude: 35.7517,
    category: 'historical',
    image_url: 'https://example.com/ajloun.jpg',
    rating: 4.2,
    is_active: true,
    created_at: new Date().toISOString(),
  },
  {
    id: 'karak-castle',
    name: 'Karak Castle',
    name_ar: 'قلعة الكرك',
    description: 'Crusader castle',
    description_ar: 'قلعة صليبية',
    latitude: 31.1848,
    longitude: 35.7047,
    category: 'historical',
    image_url: 'https://example.com/karak.jpg',
    rating: 4.1,
    is_active: true,
    created_at: new Date().toISOString(),
  },
  {
    id: 'rainbow-street',
    name: 'Rainbow Street',
    name_ar: 'شارع الرينبو',
    description: 'Cultural street in Amman',
    description_ar: 'شارع ثقافي في عمان',
    latitude: 31.9515,
    longitude: 35.9239,
    category: 'cultural',
    image_url: 'https://example.com/rainbow-street.jpg',
    rating: 4.0,
    is_active: true,
    created_at: new Date().toISOString(),
  },
];

// Error Messages
export const ERRORS = {
  NETWORK_ERROR: 'Network connection error',
  AUTHENTICATION_ERROR: 'Authentication failed',
  VALIDATION_ERROR: 'Please check your input',
  UNKNOWN_ERROR: 'An unexpected error occurred',
  LOCATION_PERMISSION_DENIED: 'Location permission denied',
  CAMERA_PERMISSION_DENIED: 'Camera permission denied',
};

// Success Messages
export const SUCCESS_MESSAGES = {
  LOGIN_SUCCESS: 'Successfully logged in',
  REGISTRATION_SUCCESS: 'Account created successfully',
  PROFILE_UPDATED: 'Profile updated successfully',
  ITINERARY_ADDED: 'Added to itinerary',
  ITINERARY_REMOVED: 'Removed from itinerary',
  DATA_SYNCED: 'Data synchronized successfully',
};